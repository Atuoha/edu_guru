...
class HomeRepo {
  ...
  void init() async {
    ...
    if (result.code == 200) {
      ...
      populateCourseList(courseList: courseList);

      for (CourseItem item in courseList) {
        print(
            "Course title: ${item.title}, Course Price: ${item.price}, Course Description: ${item.description}, Course Thumbnail: ${item.thumbnail} ");
      }
